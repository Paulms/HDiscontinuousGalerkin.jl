abstract type FiniteElement{dim,shape,QuadOrder,GeomOrder} end
...
"""
gradient_value(ip::Lagrange{dim,shape<:AbstractRefShape,order}, k::Int, ξ::Vec{dim,T}) where {dim,shape,order, T}
Compute value of Continuous Lagrange Finite Element basis `j` derivative at point ξ
on the reference shape `shape`
"""
function gradient_value(ip::ContinuousLagrange{dim}, k::Int, ξ::Vec{dim,T}) where {dim, T}
    if k >getnbasefunctions(ip);throw(ArgumentError("no shape function $k for interpolation $ip"));end
    gradient(ξ -> value(ip, k, ξ), ξ)
end

function spatial_nodal_coordinate(fe::ContinuousLagrange{dim}, n_point::Int, x::AbstractVector{Vec{dim,T}}) where {dim,T}
...
function spatial_nodal_coordinate(fe::ContinuousLagrange{dim,shape,1,1}, n_point::Int, x::AbstractVector{Vec{dim,T}}) where {dim,shape,T}
