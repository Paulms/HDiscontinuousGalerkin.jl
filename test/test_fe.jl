...
@test HDG.gettopology(finiteElement) == Dict(0=>3,1=>3,2=>0)
@test HDG.getgeomtopology(finiteElement) == Dict(0=>3,1=>0,2=>0)
...
for i in 1:3
    @test HDG.spatial_nodal_coordinate(finiteElement, i, x) == y[i]
end
...
@test HDG.gettopology(finiteElement) == Dict(0=>3,1=>0,2=>0)
for i in 1:3
    @test HDG.spatial_nodal_coordinate(finiteElement, i, x) == x[i]
end
...
ξ_ref = Vec{2}((0.5,0.5))
for j = 1:3
    @test abs(ref_value(j,ξ_ref) - value(finiteElement,j,ξ_ref)) < eps(Float64)
    @test norm(gradient(ξ -> ref_value(j, ξ), ξ_ref) - gradient_value(finiteElement,j,ξ_ref), Inf) < eps(Float64)
end
...
ξ_ref = Vec{1}((0.5,))
finiteElement = ContinuousLagrange{1,RefTetrahedron,1}()
@test HDG.gettopology(finiteElement) == Dict(0=>2,1=>0)
for j = 1:2
